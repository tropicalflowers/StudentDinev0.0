...
/* --- Restaurant dataset (24 venues + menus) --- */
const RESTAURANTS = [
  {"id":"tadka-house","name":"Tadka House","type":"Indian","img":"","menu":{
    "starters":[{"n":"Paneer Tikka","d":"Smoky cottage cheese skewers","p":180,"img":""},{"n":"Veg Pakora","d":"Crispy fritters with chutney","p":120,"img":""},{"n":"Chicken Tandoori","d":"Chargrilled, spiced","p":260,"img":""}],
    "drinks":[{"n":"Masala Chai","d":"Spiced Indian tea","p":40,"img":""},{"n":"Mango Lassi","d":"Sweet yogurt blend","p":90,"img":""},{"n":"Buttermilk","d":"Salted chaas","p":50,"img":""}],
    "main":[{"n":"Butter Chicken","d":"Creamy tomato gravy","p":280,"img":""},{"n":"Dal Makhani","d":"Slow-cooked black lentils","p":220,"img":""},{"n":"Veg Thali","d":"Assorted Indian platter","p":250,"img":""}]
  }},
...
const applySearchAndFilters = () => {
  ...
  RESTAURANTS.forEach(rest => {
    Object.values(rest.menu || {}).forEach(section => {
      if (Array.isArray(section)) {
        section.forEach(item => {
          const matchesSearch = !query || item.n.toLowerCase().includes(query) || 
                                item.d.toLowerCase().includes(query) || 
                                rest.name.toLowerCase().includes(query);
          const matchesType = !filterType || item.type === filterType;
          const matchesCategory = !filterCategory || item.category === filterCategory;
          const matchesPrice = !filterPrice || parseInt(item.p) <= parseInt(filterPrice);
          const matchesRating = !filterRating || (item.rating && parseFloat(item.rating) >= parseFloat(filterRating));
          
          if (matchesSearch && matchesType && matchesCategory && matchesPrice && matchesRating) {
            results.push({...item, restaurant: rest.name, restaurantId: rest.id});
          }
        });
      }
    });
  ...
  if (results.length === 0) {
    ...
    resultsList.innerHTML = '<p style="color: #999;">No items found matching your filters</p>';
    return;
  }

  searchResults.style.display = "block";
  resultsList.innerHTML = results.map(item => `
    <div style="background: rgba(0,212,255,0.1); padding: 12px; border-radius: 6px; border: 1px solid rgba(0,212,255,0.3); margin-bottom: 8px;">
      <div style="color: white; font-weight: bold; font-size: 14px;">${item.n}</div>
      <div style="color: #999; font-size: 12px; margin-top: 4px;">${item.d}</div>
      <div style="color: #666; font-size: 12px; margin-top: 2px;">${item.restaurant}</div>
      <div style="color: #666; font-size: 12px; margin-top: 2px;">${item.type === 'veg' ? '🥬 Veg' : '🍗 Non-Veg'} • ${item.category}</div>
      <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 8px;">
        <span style="color: #4ade80; font-weight: bold;">₹${item.p}</span>
        <button class="btn" onclick="openMenuWindow('${item.restaurantId}')">Add</button>
      </div>
    </div>
  `).join('');
};
...
const searchInput = $("#searchMenuInput");
if (searchInput) {
...
if (filterCategory) filterCategory.addEventListener('change', applySearchAndFilters);
if (filterPrice) filterPrice.addEventListener('change', applySearchAndFilters);
if (filterRating) filterRating.addEventListener('change', applySearchAndFilters);
...
async function openMessHistory(){
  try {
    ...
    alert('Mess Booking History:\n\n'+lines);
  } catch(e) { alert('Could not connect to backend.'); }
}
